...
#define RPI 2 // signal to the raspberry pi pin 37 that it should start the shutdown sequence
...
// the voltage on the verter shows it is over 4.7 volts into the VIN/GND pins... 
  ...
  lowbatcycles--;
  if (lowbatcycles < 0) { lowbatcycles = 0; }
 */
  if (light > 100) { // light
 /*   if (lowbatcycles != 0) { 
       delay(1000);
    ...
    ++litcount;
    snprintf(SERIAL_BUFFER, 1023, "good light detected: %d", litcount);
    ...
    ++dimcount;
    snprintf(SERIAL_BUFFER, 1023, "low light detected: %d", dimcount);
    Serial.println(SERIAL_BUFFER);
